...
def get_models():
    ...
    global model, second_model
    if model is None:
        print("🧠 Loading ConvNeXt Tiny classification model...")
...
description_box = html.Div([
    html.H4("Description"),
    html.P("AI-powered tool for benthic species identification in marine imagery using classification and detection models.")
], className='info-box')

instruction_box = html.Div([
    html.H4("Instructions"),
    html.P("Upload an underwater image below. After upload, you’ll see predictions and bounding boxes. Visit Gallery to review results.")
], className='info-box')
...
def gallery_page():
    if not image_store:
        return html.Div([
            banner,
            dbc.Container([
                html.H3("Gallery of Uploaded Images", className='text-center', style={'color': 'white', 'marginTop': '50px'}),
    ...
    for item in image_store:
        gallery_items.append(html.Div([
            ...
            html.Img(src=item['original'], style={'maxWidth': '220px'}),
            html.Img(src=item['boxed'], style={'maxWidth': '220px', 'marginLeft': '10px'})
    ...
    return html.Div([
        ...
        dbc.Container([
            ...
            html.Div(gallery_items, style={'display': 'flex', 'flexWrap': 'wrap', 'justifyContent': 'center', 'gap': '20px'})
        ])
    ])
